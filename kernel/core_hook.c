		     ...
		     unsigned long arg4, unsigned long arg5)
{
	...
	if (current_uid().val == 0) {
		...
		if (arg2 == CMD_SUSFS_ADD_SUSFS_SUS_MEMFD) {
			if (!ksu_access_ok((void __user*)arg3, sizeof(struct st_susfs_sus_memfd))) {
				pr_err("susfs: CMD_SUSFS_ADD_SUSFS_SUS_MEMFD -> arg3 is not accessible\n");
				return 0;
			}
			if (!ksu_access_ok((void __user*)arg5, sizeof(error))) {
				pr_err("susfs: CMD_SUSFS_ADD_SUSFS_SUS_MEMFD -> arg5 is not accessible\n");
				return 0;
			}
			error = susfs_add_sus_memfd((struct st_susfs_sus_memfd __user*)arg3);
			pr_info("susfs: CMD_SUSFS_ADD_SUSFS_SUS_MEMFD -> ret: %d\n", error);
			copy_to_user((void __user*)arg5, &error, sizeof(error));
			return 0;
		}
